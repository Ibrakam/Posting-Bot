...
print(data.get_all_chanels_name_from_user_id(user_id=889121031))

"""Some Func"""


# def chat_gpt_completion(user_message):
#     completion = client.chat.completions.create(
#         model="gpt-3.5-turbo",
#         messages=[
#             {"role": "system", "content": "Ты профессиональный Контент-менеджер"},
#             {"role": "user", "content": f"напиши пост на тему {user_message}"},
#         ]
#     )
...
@route.callback_query(F.data == 'next')
async def return_to_main_menu(callback: types.CallbackQuery, state: FSMContext) -> None:
...
async def publish_posts_ai(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if callback.data == 'further_ai':
        if len(message_to_post) > counting:
            ...
            if photo[counting]:
                await callback.message.edit_text(f'''{message_to_post[counting][1]} [cсылка на фото](%s)'''%photo[counting], reply_markup=keyboards.post_ai(photo[counting]), parse_mode='Markdown')
            else:
                await callback.message.edit_text(f'''{message_to_post[counting][1]}''', reply_markup=keyboards.post_ai(), parse_mode='Markdown')
        ...
        if photo:
            ...
            print(photo)
            await callback.message.edit_text(f'''{message_to_post[counting][1]} ''', reply_markup=keyboards.post_ai(photo[counting]), parse_mode='Markdown')
...
async def update_counting_state(message: types.Message, state: FSMContext) -> None:
    ...
    if photo[user_counting[user_id]]:
        await message.answer(f'{message.text} [cсылка на фото](%s)'%photo[user_counting[user_id]], parse_mode='Markdown', reply_markup=keyboards.post_ai(photo[user_counting[user_id]]))
        await state.clear()
    else:
        await message.answer(f'{message.text}', parse_mode='Markdown', reply_markup=keyboards.post_ai(photo[user_counting[user_id]]))
        await state.clear()
